...
WalletDashboard.getLayout = (page) => {
            ...
            <div className="grid grid-cols-2 max-w-6xl m-auto px-10">
                <div className="flex items-center">
                    ...
                    <div className="relative ml-3">
                        ...
                        <input type="search" id="default-search" className="block p-4 pl-10 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border-hidden focus:border-fuchsia-400" placeholder="Search" required onChange={search}/>
                    </div>
                </div>
                <div className="flex items-center justify-end">
                    <div className="flex items-center">
                        <div className="flex items-center h-5">
                            <input id="params" type="checkbox" value="" className="w-4 h-4 border border-gray-300 rounded bg-gray-50 focus:ring-3 focus:ring-blue-300" required />
                        </div>
                        <label htmlFor="params" className="ml-2">Params Only</label>
                    </div>
                    <div className="flex items-center ml-3">
                        <div className="flex items-center h-5">
                            <input id="hidezero" type="checkbox" value="" className="w-4 h-4 border border-gray-300 rounded bg-gray-50 focus:ring-3 focus:ring-blue-300" required />
                        </div>
                        <label htmlFor="hidezero" className="ml-2">Hide zero balances</label>
                    </div>
        ...
        <div className="max-w-6xl m-auto">
                ...
                <div>
                    {
                        display.slice((page - 1) * perpage, page * perpage).map((item, index) => <div className="group grid grid-cols-2 lg:grid-cols-3 px-10 hover:bg-slate-100 rounded-2xl" key={"currency" + index}>
                            <div className="my-4 flex items-center">
                                {item.name=="ghst" ? <div className="rounded-full w-8 h-8 flex items-center justify-center bg-slate-300">G</div>: <img className="w-8 h-8" alt="Image" src={"/images/currency/" + item.name+".svg"} />}
                                <div className="ml-3">
                                    ...
                                    {item.plasma && <span className="bg-sky-100 text-teal-600 text-center px-2 text-xs rounded ml-3">PLASMA</span>}
                                </div>
                            </div>
            ...
            <div className="px-10 mt-3 flex">
                <ul className="inline-flex items-center">
                    ...
                    <li className="ml-2">
                        <a onClick={() => {if(page < total) setPage(page + 1)}} className="rounded-lg block p-2 leading-tight text-gray-500 bg-white rounded-lg border border-gray-300 hover:bg-gray-100 hover:text-gray-700">
